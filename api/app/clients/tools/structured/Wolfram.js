...
class WolframAlphaAPI extends Tool {
  ...
  async fetchRawText(url) {
    try {
      ...
      trackException(error, { event: TelemetryEvents.ERROR_SEARCH_TOOL, tool: 'wolfram' });
      throw error;
    }
  }
  ...
  async _call(data) {
    try {
      ...
      if (error.response && error.response.data) {
        ...
        trackException(error, { event: TelemetryEvents.ERROR_SEARCH_TOOL, tool: 'wolfram', phase: 'api_error_data' });
        return error.response.data;
      } else {
        logger.error('[WolframAlphaAPI] Error querying Wolfram Alpha', error);
        trackException(error, { event: TelemetryEvents.ERROR_SEARCH_TOOL, tool: 'wolfram', phase: 'query' });
