  ...
  async sendMessage(message, opts = {}) {
    ...
    if (!isEdited && !this.skipSaveUserMessage) {
      ...
      userMessagePromise = this.saveMessageToDatabase(userMessage, saveOptions, user).catch(
        (err) => {
          ...
          trackException(err, { event: TelemetryEvents.ERROR_BASE_CLIENT, phase: 'save_user_message' });
          return {};
        },
      );
    ...
    if (this.options.attachments) {
      try {
        ...
        trackException(error, { event: TelemetryEvents.ERROR_BASE_CLIENT, phase: 'map_attachments' });
      }
    }
